...
struct StatusProvider: TimelineProvider {
    func placeholder(in context: Context) -> StatusEntry {
        StatusEntry(
            date: .now,
            summary: Policy.Summary(openNames: ["Chess"], openUntil: .now.addingTimeInterval(3600), blockedCount: 3)
        )
    }
...
struct MacStatusWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: "FurloughMacStatus", provider: StatusProvider()) { entry in
