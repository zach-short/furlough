...
- **Across devices.** Drop anchor on the iPhone and the Mac locks too; drop it on the Mac and the iPhone locks; scan the tag on the iPhone and both release. What travels is the Anchor's state alone — down or not, since when, until when — through your own iCloud key-value store, with no account of Furlough's and no server. Each device keeps its own list, because a Screen Time token means nothing off the phone that minted it and a bundle identifier means nothing on it. The Mac has no tag reader, so it can drop but never release, and it refuses to drop until it has heard from your iPhone, so it can never lock itself with no key. It refuses for the same reason when the Mac is signed out of iCloud or has iCloud Drive off, since your iPhone's tag could not reach it then either — being heard from once is remembered, and would otherwise be trusted from an account the Mac has since left. Both devices say so on the Anchor screen rather than failing quietly: signing out is done in Settings, where Furlough is not, so each checks on every launch, whenever iCloud reports the account moved, and the Mac every half minute besides. Nothing already holding is lifted by it. A device that cannot reach iCloud keeps its last state, and a Mac hears of a drop within about half a minute of it reaching iCloud.

There is no switch for any of this and nothing to set up: the transport is your own Apple Account, so two devices signed into the same one are already linked, and the only way out is iCloud itself.
...
| The Anchor | Its own list of apps and sites, chosen from what is on the Mac, or everything except a list, with the same scope as the phone. **Drop anchor** locks the Mac and, through iCloud, the iPhone; anchored apps are quit and anchored sites go to the shield page. No tag reader here, so the Mac never releases: scanning the tag on the iPhone releases both |
