...
/// The preview sentence shown before a rule is saved — a preview that lies is worse than none.
@Suite struct ConsequenceTests {
    ...
    // A night is stored as an evening plus next-day morning, read back as one span: nothing
    // shuts at midnight, so nothing says it does.
    ...
    // "Blocked all day" would be false to someone holding the phone at 2 AM.
    ...
    // A first rule replaces nothing, so undoing it means removing the whole of it.
    ...
    // Says both numbers, so the end of the trial week is never a surprise.
    ...
    // A tightening against "unrestricted", so both halves get said.
